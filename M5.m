...
tic
g = imguidedfilter(img)
result = img+5*(img-g)
[J,s_tildeT,tildeT] = rank1_enhancement(img,omega);
r=CT(double(J)/255,double(result)/255);
toc
...
% 读取图像
%TestImage = im2double(imread("C:\Users\Lenovo\Desktop\Sandstorm.jpg"));
%ProcessedImage = im2double(imread("C:\Users\Lenovo\Desktop\Sandstorm_M1.png"));
...
%[g, tilde_g] = adaptive_manifold_filter(hazedImage, sigma_s, sigma_r);
%result = hazedImage+5*(hazedImage-g)
%[J,s_tildeT,tildeT] = rank1_enhancement(hazedImage,omega);
%r=CT(double(J)/255,double(img)/255);
